...
|test7 |`forall (a b : bool) (x y : Z) (ite a (ite b (= (+ (* 2 x) 1) (+ (* 2 y) 1)) (= (+ (* 2 x) 1) (* 2 y))) (ite b (= (* 2 x) (+ (* 2 y) 1)) (= (* 2 x) (* 2 y)))) => (and (=> a b) (=> b a) (= x y))` | QF_UFLIA | Failure      | (Checker returns false) Fails at step `t64` as a consequence of `flatten_subproof` of subproofs created by a bool_simplify (at step `t2`)  | Failure     | (Checker returns false) Failure because of step `(t7, HoleAST, (cl  ((((1 + (2 * op_2)) = (1 + (2 * op_3))) = (op_2 = op_3)))), [], [ ARITH_POLY_NORM (((1 + (2 * op_2)) = (1 + (2 * op_3))) = (op_2 = op_3))])`, Micromega cannot solve this |
