...
Section Int31_Specs.
 ...
 replace (z mod wB) with z; auto with zarith.
  ...
  (* Test Unit *) (* mul_1_l 23 *) (* Result: timeout *)
  (* Require Import SMTCoq.SMTCoq.
  fold mul' in *. Fail timeout 20 (time abduce 6). *)
(* Output: Tactic call ran for 16.741 secs (0.103u,0.052s) (failure)
...
(* Test Unit *) (* mul_1_l 24 *) (* Result: timeout *)
(* Require Import SMTCoq.SMTCoq.
 rewrite Z.mul_add_distr_r. rewrite <- Z.add_assoc.
 fold mul' in *. f_equal. f_equal. Fail timeout 20 (time abduce 1). *)
(* Output: Discarding the following lemma (unsupported): ((S n <= 31)%nat : Prop)
[Lemma,SMTCoq plugin]
The command has indeed failed with message:
Tactic failure: [Proofview.tclTIMEOUT] Tactic timeout!. *)

(* Test Unit *) (* mul_add_distr_r 5 *) (* Result: timeout *)
(* Require Import SMTCoq.SMTCoq. fold mul' in *. Fail timeout 20 (time abduce 1). *)
...
Tactic failure: [Proofview.tclTIMEOUT] Tactic timeout!.*)
 ...
 change [|0|] with 0%Z.
(* Test Unit *) (* mul_0_l 1 *) (* Result: timeout *)
(* change [|0|] with 0%Z.
Require Import SMTCoq.SMTCoq. 
...
(* Test Unit *) (* mul_1_l 25 *) (* Result: exception *)
(* Require Import SMTCoq.SMTCoq.
 change (2^(Z.of_nat 0)) with 1.
 Fail timeout 20 (time abduce 1). *)
(* Output: Tactic call ran for 0.036 secs (0.036u,0.s) (failure)
The command has indeed failed with message:
Anomaly
"Uncaught exception Failure("Atom (op_6 (aka nshiftl) op_0 (aka x) (op_5 (aka S) op_4 (aka n))) is not of the expected type")." *)
 ...
 + (* Test Unit *) (* mul_0_r 2 *) (* Result: abduce success *)
   (* assert (0 + Z.succ j <= ((Z.succ j + 0) / 2 + 1) ^ 2) by admit.
   Require Import SMTCoq.SMTCoq. fold mul' in *.
   Fail timeout 20 (time abduce 1).
   assert ((mul' (Z.succ j) 0) = 0). { Search (_ * 0 = 0). apply Z.mul_0_r. } smt. *)
(* Tactic call ran for 1.514 secs (0.029u,0.019s) (failure)
The command has indeed failed with message:
cvc5 returned SAT.
The solver cannot prove the goal, but one of the following hypotheses (printed in Prop, but the corresponding Boolean versions also apply) would make it provable:
(mul' (Z.succ j) 0) = 0 *)
 ...
 generalize (sqr_pos (Z.succ j / 2)) (quotient_by_2 (Z.succ j));
   unfold Z.succ.
 (* Test Unit *) (* mul_add_distr_r 6 *) (* Result: timeout *)
 (* rewrite Z.pow_2_r.
 ...
 (* Test Unit *) (* mul_add_distr_r 7 *) (* Result: timeout *)
 (* unfold Z.succ.
 rewrite Z.pow_2_r. rewrite Z.pow_2_r.
 ...
 (* Test Unit *) (* mul_1_l 26 *) (* Result: abduce success *)
 (* Require Import SMTCoq.SMTCoq. fold mul' in *.
 rewrite Z.mul_1_l. rewrite Z.mul_1_r. rewrite Z.mul_1_l. 
 rewrite Z.mul_1_r. Fail timeout 20 (time abduce 1).
 assert ((mul' 1 k) = k). { Search (1 * _ = _). apply Z.mul_1_l. }
 smt. *)
(* Tactic call ran for 0.566 secs (0.035u,0.011s) (failure)
The command has indeed failed with message:
cvc5 returned SAT.
The solver cannot prove the goal, but one of the following hypotheses (printed in Prop, but the corresponding Boolean versions also apply) would make it provable:
(mul' 1 k) = k*)
...
Tactic failure: [Proofview.tclTIMEOUT] Tactic timeout!.*)
 ...
 (* Test Unit *) (* mul_1_l 27 *) (* Result: timeout *)
 (* rewrite Z.pow_2_r in H2 |- *. rewrite Z.mul_add_distr_r.
 rewrite Z.mul_add_distr_l.
 Require Import SMTCoq.SMTCoq. fold mul' in *.
 rewrite Z.mul_1_r. Fail timeout 20 (time abduce 1). *)
(* Output: The command has indeed failed with message:
...
(* Test Unit *) (* mul_add_distr_r 9 *) (* Result: timeout *)
(* rewrite Z.pow_2_r in H2 |- *.
 assert (i / 2 * (i / 2 + 1) + 1 * (i / 2 + 1) = i / 2 * (i / 2) + i / 2 * 1 + 1 * (i / 2 + 1)) by admit.
assert (1 * (i / 2 + 1) = i / 2 + 1) by admit.
assert (i / 2 * 1 = i / 2) by admit.
Require Import SMTCoq.SMTCoq.
fold mul' in *. Fail timeout 20 (time abduce 1). *)
(* Output: The command has indeed failed with message:
Tactic failure: [Proofview.tclTIMEOUT] Tactic timeout!.*)
   ...
   + rewrite spec_compare; case Z.compare_spec;
     ...
     * assert (Hf1: 0 <= phi2 ih il/ [|j|]). { apply Z.div_pos; lia. }
       ...
       { rewrite spec_add, div31_phi; change [|2|] with 2; auto.
         (* Test Unit *) (* mul_1_l 28 *) (* Result: timeout *)
         (* rewrite Z.add_comm, <- Z_div_plus_full_l by lia. 
         change (phi v30 * 2) with 
         (2 ^ Z.of_nat size). rewrite Zmod_small. 
         Require Import SMTCoq.SMTCoq. fold mul' in *.
         intros HH. 
         Fail timeout 20 (time abduce 1). *)
(* Output: The command has indeed failed with message:
...
(* Test Unit *) (* mul_add_distr_r 10 *) (* Result: exception *)
(* Require Import SMTCoq.SMTCoq. 
Fail timeout 20 (time abduce 1). *)
(* Output: Tactic call ran for 0.091 secs (0.027u,0.02s) (failure)
The command has indeed failed with message:
Solver error: (error argument type is not a subtype of the function's argument type:
argument:  op_5
has type:  (-> Tindex_1 Int)
not subtype: Int
in term : (op_10 (- 1) (op_2 2 (op_1 op_0)) (to_int op_5) (op_9 op_8)) ).*)
 ...
 * contradict Hs2; apply Z.le_ngt.
 ...
 (* Test Unit *) (* mul_add_distr_r 11 *) (* Result: timeout *)
 (* Require Import SMTCoq.SMTCoq. fold mul' in *.
 Fail timeout 20 (time abduce 2). *)
(* Output: Tactic call ran for 9.582 secs (0.051u,0.032s) (failure)
The command has indeed failed with message:
cvc5 returned SAT.
 ...
 * split.
 ...
 { apply Z.le_antisymm. lia.
   ...
   + (* Test Unit *) (* mul_1_l 29 *) (* Result: timeout *)
   (* fold mul' in *. Require Import SMTCoq.SMTCoq. 
   Fail timeout 20 (time abduce 1). *)
(* Output: The command has indeed failed with message:
Tactic failure: [Proofview.tclTIMEOUT] Tactic timeout!. *)

(* Test Unit *) (* mul_add_distr_r 12 *) (* Result: timeout *)
   (* fold mul' in *. Require Import SMTCoq.SMTCoq. 
   Fail timeout 20 (time abduce 1). *)
(* Output: The command has indeed failed with message:
Tactic failure: [Proofview.tclTIMEOUT] Tactic timeout!. *)
   ...
   auto with zarith. 
   + case (phi_bounded il1); intros Hpil2 _. nia. }
