...
Proof.
  ...
  - (*rel_prime*)
    ...
    * intros x Ha Hb.
      ...
      exists x'.
      (* Test Unit *) (* mul_reg_l 1 *) (* Result: timeout *)
      (* assert (g * 1 = g * (x' * x)) by admit.
...
Tactic failure: [Proofview.tclTIMEOUT] Tactic timeout!. *)
      apply Z.mul_reg_l with g.
      + apply Hg0.
      + rewrite Hx at 1. 
      (* Additional *)
      (* fold mul' in *. 
        assert ((x' *' (g *' x)) = (g *' (x' *' x))) by admit.
...
Tactic failure: [Proofview.tclTIMEOUT] Tactic timeout!.*)
  ...
  - (* rel_prime *)
    ...
    * intros x Hc Hd.
      ...
      exists x'.
      (* Test Unit *) (* mul_reg_l 2 *) (* Result: timeout *)
      (* assert (g' * 1 = g' * (x' * x)) by admit.
      fold mul' in *. Fail timeout 20 (time abduce 1). *)
(* Output: The command has indeed failed with message:
Tactic failure: [Proofview.tclTIMEOUT] Tactic timeout!.*)
      ...
      + rewrite Hx at 1. 
        (* Additional *)
        (* fold mul' in *. assert ((x' *' (g' *' x)) = (g' *' (x' *' x)))
        by admit. Fail timeout 20 (time abduce 1). *)
(* Output: The command has indeed failed with message:
Tactic failure: [Proofview.tclTIMEOUT] Tactic timeout!. *)
        ring.
  - apply Z.lt_gt.
  ...
  - (* Test Unit *) (* mul_reg_l 3 *) (* Result: timeout *)
  (* assert (g * g' * (aa * dd) = g * g' * (bb * cc)) by admit.
    fold mul' in *. Fail timeout 20 (time abduce 1). *)
(* Output: The command has indeed failed with message:
Tactic failure: [Proofview.tclTIMEOUT] Tactic timeout!. *)
...
(* Output: Tactic call ran for 14.544 secs (0.063u,0.024s) (failure)
