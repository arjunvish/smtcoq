...
(* Alternate solution, but we're not counting these in experiments:
 + smt. + smt. + smt. + smt. 
 + intros. fold mul' in *. Fail timeout 20 (time abduce 2).
   assert ((mul' y x) = (mul' x y)). { Search (_ * _ = _ * _)%Z. 
   apply Z.mul_comm. } smt.
(*Tactic call ran for 5.586 secs (0.008u,0.016s) (failure)
...
*)
 ...
 + smt. 
Qed. *)
 exact Z.add_0_l. exact Z.add_comm. exact Z.add_assoc. 
 (* Test Unit *) (* mul_1_l 30 *) (* Result: smt success *)
 (* smt. *)
 exact Z.mul_1_l. 
 exact Z.mul_comm. exact Z.mul_assoc.
 (* Test Unit *) (* mul_add_distr_r 13 *) (* Result: timeout *) 
 (* intros. fold mul' in *. Fail timeout 20 (time abduce 1). *)
 (* Output:  The command has indeed failed with message:
Tactic failure: [Proofview.tclTIMEOUT] Tactic timeout!.*)
 exact Z.mul_add_distr_r. 
 trivial. exact Z.sub_diag.
Qed.
