...
Proof.
  ...
  { destruct (le_lt_dec i j).
    ...
    + (* Test Unit *) (* mul_1_l 11 *) (* Result: timeout *) 
      (* Require Import SMTCoq.SMTCoq.
      Fail timeout 20 (time abduce 2). *)
(* Output: Tactic call ran for 5.961 secs (0.02u,0.036s) (failure)
...
(Pos.mul 4 n) = (Pos.mul 4 p) *)
    ...
    apply inject_Q_le. unfold Qle, Qnum, Qden.
    (* Test Unit *) (* mul_1_l 12 *) (* Result: timeout *)
    (* Require Import SMTCoq.SMTCoq.
    Fail timeout 20 (time abduce 2). *)
    (* Output: Tactic call ran for 5.051 secs (0.022u,0.035s) (failure)
The command has indeed failed with message:
cvc5 returned SAT.
The solver cannot prove the goal, but one of the following hypotheses (printed in Prop, but the corresponding Boolean versions also apply) would make it provable:
q = n
(Pos.mul 4 n) = (Pos.mul 4 q) *)
    rewrite Z.mul_1_l, Z.mul_1_l. apply Pos2Z.pos_le_pos.
  ...
  + (* Test Unit *) (* mul_1_l 13 *) (* Result: timeout *)
   (* Require Import SMTCoq.SMTCoq.
...
Tactic failure: [Proofview.tclTIMEOUT] Tactic timeout!. *)
  ...
  - (* Test Unit *) (* mul_1_l 14 *) (* Result: timeout *)
   (* Require Import SMTCoq.SMTCoq.
    Fail timeout 20 (time abduce 1). *)
  (* Output: Tactic call ran for 5.091 secs (0.006u,0.03s) (failure)
The command has indeed failed with message:
cvc5 returned SAT.
The solver cannot prove the goal, but one of the following hypotheses (printed in Prop, but the corresponding Boolean versions also apply) would make it provable:
(Pos.mul 3 n) = (Pos.mul 4 n) *)
    rewrite Z.mul_1_l, Z.mul_1_l.
  ...
  - apply (CReal_le_lt_trans _ (inject_Q (1#4*q))).
    ...
    (* Test Unit *) (* mul_1_l 15 *) (* Result: timeout *)
    (* Require Import SMTCoq.SMTCoq.
Fail timeout 20 (time abduce 1). *)
(* Output: The command has indeed failed with message:
Tactic failure: [Proofview.tclTIMEOUT] Tactic timeout!. *)
...
Proof.
  ...
  assert (CReal_abs (xn i - xn j) <= inject_Q (1 # 4 * 2^n')).
  {
    ...
    apply (CReal_le_trans _ _ _ (imaj i j (le_refl _) l)).
    apply inject_Q_le. unfold Qle, Qnum, Qden.
(* Test Unit *) (* mul_1_l 16 *) (* Result: exception *)
(* Require Import SMTCoq.SMTCoq.
Fail timeout 20 (time abduce 1). *)
(* Output: Tactic call ran for 0.04 secs (0.04u,0.s) (failure)
The command has indeed failed with message:
Anomaly
"Uncaught exception Failure("Atom op_2 (aka q') is not of the expected type")."*)
    rewrite Z.mul_1_l, Z.mul_1_l. apply Pos2Z.pos_le_pos.
    subst; apply Pos.mul_le_mono_l, Pos_pow_le_mono_r, CReal_from_cauchy_cm_mono, Hp.
    apply le_S, le_S_n in l.
    apply (CReal_le_trans _ _ _ (jmaj i j l (le_refl _))).
    apply inject_Q_le. unfold Qle, Qnum, Qden.
(* Test Unit *)  (* mul_1_l 17 *) (* Result: exception *)
(* Require Import SMTCoq.SMTCoq.
Fail timeout 20 (time abduce 1). *)
(* Output: Tactic call ran for 0.041 secs (0.037u,0.004s) (failure)
The command has indeed failed with message:
Anomaly
"Uncaught exception Failure("Atom op_2 (aka q') is not of the expected type")." *)
    rewrite Z.mul_1_l, Z.mul_1_l. apply Pos2Z.pos_le_pos.
...
Proof.
  ...
  specialize (imaj j i (le_trans _ _ _ (Nat.le_max_r _ _) H0) (le_refl _)).
    ...
    apply inject_Q_le.
    unfold Qle, Qnum, Qden.
(* Test Unit *) (* mul_1_l 18 *) (* Result: exception *)
(* Require Import SMTCoq.SMTCoq.
Fail timeout 20 (time abduce 1). *)
(* Output: Tactic call ran for 0.01 secs (0.001u,0.008s) (failure)
The command has indeed failed with message:
Anomaly
"Uncaught exception Failure("Atom op_0 (aka i') is not of the expected type")." *)
    rewrite Z.mul_1_l, Z.mul_1_l.
