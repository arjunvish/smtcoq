...
Proof.
  ...
  - right. apply (CR_cv_le (fun n => CR_of_Q R (yn n))
    ...
    + intro p. exists (Pos.to_nat p). intros.
      ...
      (* Test Unit *) (* mul_1_l 6 *) (* Result: timeout *) 
    (* Require Import SMTCoq.SMTCoq.
      Fail timeout 20 (time abduce 2).
      Tactic call ran for 1.015 secs (0.006u,0.011s) (failure)
...
(Pos.of_nat (S i)) = p *)
    ...
    + apply (CR_cv_proper _ (x+0)). 2: rewrite CRplus_0_r; reflexivity.
      ...
      apply CRlt_asym, (snd p0). apply CRplus_le_compat_l.
      apply CR_of_Q_le. unfold Qle, Qnum, Qden.
(* Test Unit *) (* mul_1_l 7 *) (* Result: timeout *) 
(* Require Import SMTCoq.SMTCoq.
Fail timeout 20 (time abduce 1). *)
(* Output: Tactic call ran for 1.031 secs (0.01u,0.019s) (failure)
The command has indeed failed with message:
cvc5 returned SAT.
The solver cannot prove the goal, but one of the following hypotheses would make it provable:
(Pos.of_nat (S i)) = p *)
      ...
      apply CR_of_Q_le. unfold Qle, Qnum, Qden.
(* Test Unit *) (* mul_1_l 8 *) (* Result: timeout *)
(* Require Import SMTCoq.SMTCoq.
Fail timeout 20 (time abduce 1). *)
(* Output: Tactic call ran for 1.11 secs (0.005u,0.014s) (failure)
The command has indeed failed with message:
cvc5 returned SAT.
The solver cannot prove the goal, but one of the following hypotheses would make it provable:
(Pos.of_nat (S i)) = p *)
...
(* Test Unit *) (* mul_1_l 9 *) (* Result: timeout *) 
(* Require Import SMTCoq.SMTCoq.
Fail timeout 20 (time abduce 1). *)
...
(* Test Unit *) (* mul_1_l 10 *) (* Result: timeout *)
(*Require Import SMTCoq.SMTCoq.
   Fail timeout 20 (time abduce 1). *)
(* Output: The command has indeed failed with message:
Tactic failure: [Proofview.tclTIMEOUT] Tactic timeout!. *)
