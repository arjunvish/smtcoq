...
- Z.nzsimpl. Z.order.
- rewrite (Z.mul_lt_mono_pos_r y) in GT.
 + (* Test Unit *) (* mul_1_l 32 *) (* Result: abduce success *)
   (* fold mul' in *. Fail timeout 20 (time abduce 2).
   assert ((mul' 1 y) = y). { apply Z.mul_1_l. } smt. *)
(* Output: Tactic call ran for 0.345 secs (0.014u,0.007s) (failure)
...
(mul' 1 y) = x
(mul' 1 y) = y *)
   rewrite Z.mul_1_l in GT; Z.order.
 + (* Additional *) (* smt. *) Z.order.
Qed.
...
(* assert (v * (c1 * k1 + c2 * k2) = v * (c1 * k1) + v * (c2 * k2)) by admit.
assert (v * (c1 * k1) + v * (c2 * k2) = v * c1 * k1 + v * c2 * k2) by admit.
...
Proof.
(* Test Unit *) (* mul_add_distr_r 16 *) (* Result: timeout *)
(* assert (v1 * (c1 * k1) = v1 * c1 * k1) by admit.
assert (v1 * c1 * k1 + (l1 * k1 + l2) = v1 * c1 * k1 + l1 * k1 + l2) by admit.
fold mul' in *. Fail timeout 20 (time abduce 1). *)
(* Output: The command has indeed failed with message:
Tactic failure: [Proofview.tclTIMEOUT] Tactic timeout!. *)
rewrite ?Z.mul_add_distr_r, ?Z.mul_add_distr_l, ?Z.mul_assoc.
...
Proof.
(* Test Unit *) (* mul_add_distr_r 17 *) (* Result: timeout *)
(* assert (v2 * (c2 * k2) = v2 * c2 * k2) by admit.
fold mul' in *. Fail timeout 20 (time abduce 1). *)
(* Output: The command has indeed failed with message:
Tactic failure: [Proofview.tclTIMEOUT] Tactic timeout!.*)
rewrite ?Z.mul_add_distr_r, ?Z.mul_add_distr_l, ?Z.mul_assoc.
...
Proof.
(* Test Unit *) (* mul_add_distr_r 18 *) (* Result: timeout *)
(* assert (v * (c1 + c2 * k2) = v * c1 + v * (c2 * k2)) by admit.
 assert (v * c1 + v * (c2 * k2) = v * c1 + v * c2 * k2) by admit.
 fold mul' in *. Fail timeout 20 (time abduce 1). *)
(* Output: The command has indeed failed with message:
Tactic failure: [Proofview.tclTIMEOUT] Tactic timeout!.*)
 rewrite ?Z.mul_add_distr_r, ?Z.mul_add_distr_l, ?Z.mul_assoc.
...
Proof.
(* Test Unit *) (* mul_add_distr_r 19 *) (* Result: timeout *)
(* assert (v * (c * k) = v * c * k) by admit. 
 fold mul' in *. Fail timeout 20 (time abduce 1). *)
(* Output: The command has indeed failed with message:
Tactic failure: [Proofview.tclTIMEOUT] Tactic timeout!.*)
...
Proof.
 ...
 - (* Test Unit *) (* mul_opp_r 7 *) (* Result: smt success *) (* smt. *) right. change (-1) with (-(1)).
