...
Proof.
 destruct p as [|p|p].
 - (* Test Unit *) (* mul_0_r 1 *) (* Result: smt success *) (* smt. *) now rewrite !mul_0_r.
 - apply mul_add_distr_pos.
 - apply opp_inj.
   (* Test Unit *) (* mul_opp_r 1 *) (* Result: timeout *) 
   (* rewrite opp_add_distr. fold mul' in *. 
   assert ((n + m) * - neg p = n * - neg p + m * - neg p) by admit.
   Fail timeout 20 (time abduce 1). *)
(* Output: The command has indeed failed with message:
...
Proof.
 unfold succ.
(* Test Unit *) (* mul_add_distr_r 1 *) (* Result: timeout *)
(* fold mul'. assert (mul' 1 m = m) by admit.
Fail timeout 20 (time abduce 1). *)
(* Output: The command has indeed failed with message:
Tactic failure: [Proofview.tclTIMEOUT] Tactic timeout!. *)
rewrite mul_add_distr_r.
(* Test Unit *) (* mul_1_l 1 *) (* Result: abduce success *) 
(* fold mul'.
Fail timeout 20 (time abduce 1).
assert ((mul' 1 m) = m). { Search (1 * _ = _). apply mul_1_l. } smt. *)
(* Output: Tactic call ran for 0.167 secs (0.004u,0.012s) (failure)
...
(* Test Unit *) (* mul_add_distr_r 2 *) (* Result: timeout *) 
   (* assert (2 * q * b + 1 * b + (2 *' r + 1 - b) = 2 * q * b + (1 * b + (2 * r + 1 - b))) by admit.
   assert (2 * q * b + 2 * r + 1 = 2 * q * b + (2 * r + 1)) by admit.
   assert (2 * q * b + (2 * r + 1) = 2 * q * b + (1 * b + (2 * r + 1 - b)) -> 
   2 * r + 1 = 1 * b + (2 * r + 1 - b)) by admit.
   assert (1 * b = b) by admit.
   assert (2 * r + 1 + - b = - b + (2 * r + 1)) by admit.
   assert (b + (- b + (2 * r + 1)) = b + - b + (2 * r + 1)) by admit.
   assert (b + - b = 0) by admit.
   fold mul' in *. 
Fail timeout 20 (time abduce 1). *)
(* Output: The command has indeed failed with message:
Tactic failure: [Proofview.tclTIMEOUT] Tactic timeout!. *)
   rewrite mul_add_distr_r, <- !add_assoc.
(* Test Unit *) (* mul_1_l 2 *) (* Result: abduce success *) 
   (* fold mul' in *. Fail timeout 20 (time abduce 1).
   assert ((mul' 1 b) = b). { Search (1 * _ = _). apply mul_1_l. } smt. *)
(* Output: Tactic call ran for 0.792 secs (0.017u,0.016s) (failure)
The command has indeed failed with message:
cvc5 returned SAT.
The solver cannot prove the goal, but one of the following hypotheses (printed in Prop, but the corresponding Boolean versions also apply) would make it provable:
(mul' 1 b) = b *)
   f_equal. rewrite mul_1_l. 
 ...
 - (* ~0 *)
   ...
   destruct ltb.
   trivial.
   (* Test Unit *) (* mul_add_distr_r 3 *) (* Result: timeout *) 
   (* fold mul' in *. assert ((mul' 1 b) = b) by admit.
   Fail timeout 20 (time abduce 1). *)
(* Output: The command has indeed failed with message:
Tactic failure: [Proofview.tclTIMEOUT] Tactic timeout!.*)
   rewrite mul_add_distr_r, <- !add_assoc. 
   (* Test Unit *) (* mul_1_l 3 *) (* Result: abduce success *) 
   (* fold mul' in *. Fail timeout 20 (time abduce 1).
   assert ((mul' 1 b) = b). { Search (1 * _ = _). apply mul_1_l. } smt. *)
(* Output: Tactic call ran for 0.777 secs (0.007u,0.015s) (failure)
The command has indeed failed with message:
cvc5 returned SAT.
The solver cannot prove the goal, but one of the following hypotheses (printed in Prop, but the corresponding Boolean versions also apply) would make it provable:
(mul' 1 b) = b *)
...
Proof.
 ...
 - (* neg pos *)
   ...
   rewrite (opp_add_distr _ r).
   (* Test Unit *) (* mul_opp_r 2 *) (* Result: exception *)
   (* Fail timeout 20 (time abduce 1). *)
   (* Output: "Uncaught exception Failure("Verit.tactic: can only deal with equality over bool")." *) rewrite <- mul_opp_r.
   destruct r as [ |r|r].
   + trivial.
   + rewrite opp_add_distr, mul_add_distr_l, <- add_assoc; f_equal;
    unfold sub.
    (* Test Unit *) (* mul_opp_r 3 *) (* Result: timeout *) (* Old Result: partial *)
    (* fold mul' in *. rewrite add_assoc. 
    assert ((pos b *' 1) = pos b) by admit. (* mul_1_r *)
    assert (- pos b + pos b = 0) by admit. (* add_opp_diag_l *)
    Fail timeout 20 (time abduce 1). *)
(* Output: The command has indeed failed with message:
Tactic failure: [Proofview.tclTIMEOUT] Tactic timeout!. *)
(* Old Output: assert ((mul' (pos b) (0 - 1)) + pos b = 0). { change (0 - 1) with (-1).
Search (_ * -1 ). rewrite <- BinInt.Z.opp_eq_mul_m1.
Search (- _ + _ = 0). apply ZArith.BinInt.Z.add_opp_diag_l. } verit.
Tactic call ran for 13.564 secs (0.014u,0.031s) (failure)
The command has indeed failed with message:
cvc5 returned SAT.
The solver cannot prove the goal, but one of the following hypotheses would make it provable:
(mul' b 0 - 1) + b = 0 *)
    now rewrite add_assoc, mul_opp_r, mul_1_r, add_opp_diag_l.
   + rewrite opp_add_distr, mul_add_distr_l, <- add_assoc; f_equal;
    unfold sub. rewrite add_assoc.
    (* Test Unit *) (* mul_opp_r 4 *) (* Result: partial *) 
    (* fold mul' in *. Fail timeout 20 (time abduce 1).
    assert((mul' (pos b) (0 - 1)) + (pos b) = 0). {
     change (0 - 1) with (-1). Search (_ * -1 ). 
     rewrite <- BinInt.Z.opp_eq_mul_m1.
     Search (- _ + _ = 0). apply ZArith.BinInt.Z.add_opp_diag_l. } smt. *)
(* Output: Tactic call ran for 8.948 secs (0.005u,0.027s) (failure)
The command has indeed failed with message:
cvc5 returned SAT.
The solver cannot prove the goal, but one of the following hypotheses (printed in Prop, but the corresponding Boolean versions also apply) would make it provable:
(mul' b 0 - 1) + b = 0 *)
(* Old Output: Tactic call ran for 2.336 secs (0.008u,0.023s) (failure)
The command has indeed failed with message:
cvc5 returned SAT.
The solver cannot prove the goal, but one of the following hypotheses would make it provable:
(mul' b 0 - 1) + b = 0 *)
   now rewrite mul_opp_r, mul_1_r, add_opp_diag_l.
 - (* neg neg *)
   change (neg a) with (- pos a). intros ->.
...
Proof.
 split; intros (m,H); exists (-m). 
 + rewrite mul_opp_l.
(* Test Unit *) (* mul_opp_r 5 *) (* Result: timeout *) 
(* fold mul' in *.  Fail timeout 20 (time abduce 1). *)
(* Tactic call ran for 6.791 secs (0.02u,0.021s) (failure)
The command has indeed failed with message:
cvc5 returned SAT.
The solver cannot prove the goal, but one of the following hypotheses (printed in Prop, but the corresponding Boolean versions also apply) would make it provable:
p = n && 0 = n *) now rewrite <- mul_opp_r.
 + rewrite mul_opp_l. 
(* Test Unit *) (* mul_opp_r 6 *) (* Result: timeout *) 
(* fold mul' in *. Fail timeout 20 (time abduce 2). *)
(* Output: Tactic call ran for 10.151 secs (0.005u,0.032s) (failure)
The command has indeed failed with message:
cvc5 returned SAT.
The solver cannot prove the goal, but one of the following hypotheses (printed in Prop, but the corresponding Boolean versions also apply) would make it provable:
p = n && 0 = n
...
Lemma lor_spec a b n :
 testbit (Z.lor a b) n = testbit a n || testbit b n.
Proof.
 destruct (leb_spec 0 n) as [Hn|Hn]; [|now rewrite !testbit_neg_r].
 destruct a as [ |a|a], b as [ |b|b];
  rewrite ?testbit_0_l, ?orb_false_r; trivial; unfold Z.lor;
...
Lemma land_spec a b n :
 testbit (Z.land a b) n = testbit a n && testbit b n.
Proof.
 destruct (leb_spec 0 n) as [Hn|Hn]; [|now rewrite !testbit_neg_r].
 destruct a as [ |a|a], b as [ |b|b];
  rewrite ?testbit_0_l, ?andb_false_r; trivial; unfold Z.land;
...
Lemma lxor_spec a b n :
 testbit (Z.lxor a b) n = xorb (testbit a n) (testbit b n).
Proof.
 destruct (leb_spec 0 n) as [Hn|Hn]; [|now rewrite !testbit_neg_r].
 destruct a as [ |a|a], b as [ |b|b];
  rewrite ?testbit_0_l, ?xorb_false_l, ?xorb_false_r; trivial; unfold Z.lxor;
  rewrite ?testbit_Zpos, ?testbit_Zneg, ?testbit_of_N', ?N.pos_pred_succ
   by trivial.
...
Proof. (* Additional *) (* smt. *)
 change 2 with (1+1).
 (* Test Unit *) (* mul_add_distr_r 4 *) (* Result: abduce success *) 
 (* fold mul' in *. Fail timeout 20 (time abduce 1).
 assert ((mul' 2 n) = n + n). { Search (_ + _ = 2 * _).
 symmetry. apply BinInt.Z.add_diag. }
 smt. *)
(* Output: Tactic call ran for 0.342 secs (0.004u,0.024s) (failure)
The command has indeed failed with message:
cvc5 returned SAT.
The solver cannot prove the goal, but one of the following hypotheses (printed in Prop, but the corresponding Boolean versions also apply) would make it provable:
(mul' 2 n) = n + n *)
(* Test Unit *) (* mul_1_l 4 *) (* Result: abduce success *)
  (* rewrite mul_add_distr_r. fold mul' in *. 
   Fail timeout 20 (time abduce 1).
 assert (mul' 1 n = n). { Search (1 * _ = _). 
 apply ZArith.BinInt.Z.mul_1_l. }
 smt. *)
(* Output: Tactic call ran for 0.176 secs (0.004u,0.02s) (failure)
The command has indeed failed with message:
cvc5 returned SAT.
The solver cannot prove the goal, but one of the following hypotheses (printed in Prop, but the corresponding Boolean versions also apply) would make it provable:
(mul' 1 n) = n *)
