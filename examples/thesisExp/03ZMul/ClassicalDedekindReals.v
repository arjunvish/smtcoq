...
Proof.
  ...
  - exfalso. destruct x as [f low]. unfold proj1_sig in H, q0.
    ...
    + (* Test Unit *) (* mul_1_l 5 *) (* Result: timeout *)
      (* assert (1 * Z.pos p = Z.pos p)%Z by admit.
      assert (1 * Z.pos (Pos.of_nat (S (Pos.to_nat p))) = 
              Z.pos (Pos.of_nat (S (Pos.to_nat p))))%Z by admit.
      assert ((p <= (Pos.of_nat (S (Pos.to_nat p))))%positive ->
              (Z.pos p <= Z.pos (Pos.of_nat (S (Pos.to_nat p))))%Z) 
      by admit.
      assert ((Pos.to_nat p <= Pos.to_nat (Pos.of_nat 
                (S (Pos.to_nat p))))%nat -> 
              (p <= Pos.of_nat (S (Pos.to_nat p)))%positive) by admit.
      assert (S (Pos.to_nat p) <> 0%nat ->
              Pos.to_nat (Pos.of_nat (S (Pos.to_nat p))) = 
              S (Pos.to_nat p)) by admit.
      assert ((Pos.to_nat p <= Pos.to_nat p)%nat -> 
              (Pos.to_nat p <= S (Pos.to_nat p))%nat) by admit.
      assert (Pos.to_nat p <= Pos.to_nat p)%nat by admit.
      assert (S (Pos.to_nat p) <> 0%nat) by admit.
      Require Import SMTCoq.SMTCoq.
      Fail timeout 20 (time abduce 1). *)
(* Output: 
  - When first mul_1_l is missing:
    The command has indeed failed with message:
    Tactic failure: [Proofview.tclTIMEOUT] Tactic timeout!.
  - When second mul_1_l is missing:
    The command has indeed failed with message:
    Tactic failure: [Proofview.tclTIMEOUT] Tactic timeout!.
  - When both mul_1_l are missing:
    Tactic call ran for 18.224 secs (0.125u,0.016s) (failure)
    The command has indeed failed with message:
    cvc5 returned SAT.
    The solver cannot prove the goal, but one of the following hypotheses would make it provable:
    (Pos.of_nat (S (Pos.to_nat p))) = p *) 
    do 2 rewrite Z.mul_1_l.
    apply Pos2Z.pos_le_pos. apply Pos2Nat.inj_le.
    rewrite Nat2Pos.id. apply le_S, le_refl. 
     discriminate.
