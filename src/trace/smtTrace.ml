...
let to_coq to_lit interp (cstep,
    cRes, cWeaken, cImmFlatten, cFlatten,
  ...
  let step_to_coq c =
    ...
    | Other other ->
	      begin match other with
              ...
              | Flatten f -> mklApp cFlatten [|out_c c; out_f f|]
              ...
              | EqCgr (f, fl) ->
                let res = List.fold_right (fun f l -> mklApp ccons [|mklApp coption [|Lazy.force cint|]; 
                                                      (match f with 
                                                        | Some f -> mklApp cSome [|Lazy.force cint; out_f f|] 
              ...
              | SplArith (orig,res,l) ->
                ...
                let l' = List.fold_right (fun f l -> mklApp ccons [|Lazy.force CoqTerms.micromega_coq_proofTerm; CoqInterface.micromega_dump_proof_term f; l|]) l 
                                         (mklApp cnil [|Lazy.force CoqTerms.micromega_coq_proofTerm|]) in
  ...
  let def_step = mklApp cRes [|mkInt 0; CoqTerms.mkArray (Lazy.force cint, [|mkInt 0|]) |] in
  let r = ref confl in
  let nc = ref 0 in
  ...
  let res = CoqInterface.mkTrace step_to_coq next carray clist cnil ccons cpair !nc step def_step r in
  (res, last_root, !cuts)
